...
//MODIFIED CODE: Just shortens sending a message using
// the provided SSL client c
bool sendMessage(CStringA& msg, CSSLClient* c)
{
	if (c->Send(msg.GetBuffer(), msg.GetLength()) != msg.GetLength())
	{
...
bool receiveMessage(char* buff, size_t len, CSSLClient* c)
{
	if (0 < (len = c->Recv(buff, len)))
	{
		cout << "Received '" << CStringA(buff, len) << "'" << endl;
		return true;
	}
...
int wmain(int argc, WCHAR * argv[])
{
	...
	if (b)
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			sendMessage(sentMsg, pSSLClient.get());
			...
			if (0 < (len = pSSLClient->Recv(Msg, sizeof(Msg))))
			{
				...
				//Initialize the handler we will use, and move the SSL client
				auto ch = make_unique<ClientHandler>(std::move(pSSLClient));
				ch->init();
				//Make sure to get back the ssl client to close the connection
				pSSLClient = std::move(ch->movePSSLClient());
				// Receive a second message, ignore errors
				//receiveMessage(Msg, sizeof(Msg), pSSLClient.get());
				//if (0 < (len = pSSLClient->Recv(Msg, sizeof(Msg))))
				//	cout << "Received '" << CStringA(Msg, len) << "'" << endl;
				// MODIFIED CODE:
